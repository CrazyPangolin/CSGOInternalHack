...
Hook::Hook():b_state_(false), src_(nullptr), dst_(nullptr), len_(0), ptr_to_gateway_function_ptr_(nullptr), b_init_(false)
{
	memset(original_bytes_, 0, 10);
}
void Hook::Init(BYTE* src, BYTE* dst, BYTE* ptr_to_gateway_ptr, uintptr_t len)
{
	b_init_ = true;
	b_state_ = false;
	this->src_ = src;
